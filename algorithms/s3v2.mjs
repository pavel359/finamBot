...
let arrTrades = []
let timing = '3s'
let dirName = 'algorithms/logs/s3v2.json'
function openPosition(data) {
    ...
    for (let prop of data) {
        if (timeStamp(prop['<TIME>']) <= timer2) {
            ...
            if ((share[`${ticker}`].currentPercent >= 0.014 && share[`${ticker}`].pastPercent >= 0.014) || (share[`${ticker}`].currentPercent >= 0.014 && share[`${ticker}`].pastPastPercent >= 0.014)) {
                if (share[`${ticker}`].signal != true) {
            ...
            if ((share[`${ticker}`].currentPercent <= -0.014 && share[`${ticker}`].pastPercent <= -0.014) || (share[`${ticker}`].currentPercent <= -0.014 && share[`${ticker}`].pastPastPercent <= -0.014)) {
                if (share[`${ticker}`].signal != true) {
                    ...
                    share[`${ticker}`].stopLoss = share[`${ticker}`].openPositionPrice * 1.07
                    console.log(Number(prop['<LAST>']))
                    console.log(prop['<TIME>'])
                }
                share[`${ticker}`].signal = true
